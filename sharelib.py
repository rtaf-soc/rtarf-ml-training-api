...
def maskThreat2(df):
    ...
    # countryStr = ["Russian Federation","Slovakia","China","Netherlands","Thailand"]
    countryStr = ["Russian Federation"]
    return df.assign(is_threat=pd.Series('no', index=df.index).mask(((df['ads_country_dst'].isin(countryStr)) & ((df['@timestamp'].str[11:19]<toThreatHourStr) | (df['@timestamp'].str[11:19]>fromThreatHourStr))), 'yes'))
